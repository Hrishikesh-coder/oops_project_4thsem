...
words into digit form, and classifies each by type (Date, Phone Number, Time,
License Number, etc.). The output is a structured JSON file per document
...
returns typed classification results (Date, Phone Number, Time, License Plate,
etc.). JSON serialization is now handled in a separate module.
...
class RegexParserClassifier(BaseParserClassifier):
  def classify(self, clean_text: str):
    # Regex patterns for DATE / PHONE_NUMBER / LICENSE_PLATE / TIME
    ...

class SpacyParserClassifier(BaseParserClassifier):
  def classify(self, clean_text: str):
    # spaCy entities + matcher fallback for phone/license plate
        ...
